...
$sql = "SELECT * FROM finance_request WHERE regno='$regno'";
$result = $db->query($sql);
...
if ($result->num_rows > 0) {
    ...
    $checkQuery = "SELECT * FROM finance_decline WHERE regno='$regno'";
    ...
    if ($checkResult->num_rows > 0) {
        ...
        $insertQuery = "INSERT INTO finance_approve (regno, name, department, faculty, gender, email) VALUES ('$regno', '$name', '$department', '$faculty', '$gender', '$email')";
        if ($db->query($insertQuery) === true) {
            // Delete the student record from finance_clearancerequest table
            $deleteQuery = "DELETE FROM finance_request WHERE regno='$regno'";
