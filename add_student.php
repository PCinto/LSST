    ...
    <div class="card">
      ...
      <div class="form">
        ...
        <form action="" method="POST" onsubmit="return validateForm();">
          ...
          <div class="form-group">
            <label for="entry_year">Year of Entry</label>
            <input type="number" id="entry_year" name="entry_year"  required>
          </div>
          <div class="form-group">
            <label for="exit_year">Year of Exit</label>
            <input type="number" id="exit_year" name="exit_year"  required>
          </div>
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  $currentYear = date("Y");
  if ($entry_year > $currentYear) {
    echo '<div class="error-message">Error: Year of Entry cannot be in the future.</div>';
  } else if ($exit_year < ($entry_year + 3)) {
    echo '<div class="error-message">Error: Year of Exit should be at least 3 years after the Year of Entry.</div>';
  } else {
    // Prepare and execute SQL query
    $sql = "INSERT INTO students (name, regno, faculty, department, gender, email, phone, mode, entry_year, exit_year)
            VALUES ('$name', '$regno', '$faculty', '$department', '$gender', '$email', '$phone', '$mode', '$entry_year', '$exit_year')";
...
<script>
  ...
  var yearExitInput = document.getElementById("exit_year");

  var nameError = document.getElementById("name-error");
  var regnoError = document.getElementById("regno-error");
  var departmentError = document.getElementById("department-error");
  var yearEntryError = document.getElementById("entry_year-error");
  var yearExitError = document.getElementById("exit_year-error");
